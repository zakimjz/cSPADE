...
int maxiter = 2;
int min_gap = 1;
int max_gap = static_cast<int> (INFINITY);
char use_maxgap=0;
char use_window = 0;
char memtrace = 0;
...
void parse_args(int argc, char **argv)
{
   ...
   if (use_class && count_multiple){
      cout << " cannot mix classes and multiple counting" << endl;
      exit(-1);
   }
...
//    }
//    cout << endl;
   //cout << "XXXXX" << ljoin->mcnt()->size() << " " << endl;
            ...
            if (count_multiple){
               ...
               for (k=i, l=j; k < nval1 && l < nval2; ){
                  ...
                  if (diff < min_gap){
                     ...
                     if (fk){
                        //cout << "ADD " << icid << " " << it1[k+1] << endl;
                        ljoin->ival()->add(icid);
                        ljoin->ival()->add(it1[k+1]);
                        ljoin->mcnt()->push_back(0); //increase mctr
                        fk = false;
                     }
                     ljoin->increment_mcnt();
            ...
            for (k=i, l=j; k< nval1 && l < nval2;){
               if (it1[k+1] < it2[l+1]) k+=2;
                  ...
                  if (count_multiple){
                     ...
                     ejoin->mcnt()->push_back(1); //increase mctr
                     if (use_class)
...
void make_itemset(Itemset *it, Array *ary, ullint cnt, ullint *clscnt, Array &ctr)
{
   int i;
...
void get_tmpnewf_intersect(Itemset *&ljoin, Itemset *&ejoin, Itemset *&mjoin,
                       ullint &lcnt, ullint &ecnt, ullint &mcnt,
                       Itemset *it1, Itemset *it2, int iter)
{
   ...
   Array lctr(2), ectr(2), mctr(2);

   //cout << "XXX" << ljoin << " " << ejoin << " " << mjoin << " "
   //     << MINSUPPORT << endl;
   //cout << "JOIN " << *it1 << " xxx " << *it2 << endl;
   num_intersect++;
   ...
   for (i=0,j=0; i < it1->ivalsize() && j < it2->ivalsize();){
      if (!count_multiple && (df1 > dc1 || df2 > dc2)) break;
      icid = it1->ival(i);
      jcid = it2->ival(j);
      if (icid > jcid){
            ...
            if (count_multiple){
               ...
               for (k=i, l=j; k < nval1 && l < nval2; ){
                  ...
                  if (diff < min_gap){
                     ...
                     ctr = it2->mcnt(l/2);
                     if (fk){
                        //cout << "ADD " << icid << " " << it1->ival(k+1)
                        //     << endl;
                        lary->add(icid);
                        lary->add(it1->ival(k+1));
                        lctr.add(0);
                        fk = false;
                     }
                     //cout << "YY " << ctr << " "<< l/2 << endl;
                     lctr[lctr.size()-1] += ctr;
                     lcnt+=ctr;
                     ClassInfo::TMPL[ClassInfo::getcls(icid)]+=ctr;
                     l+=2;
                  }
                  if (l >= nval2){
                     l = lpos;
            ...
            if (count_multiple){
               ...
               for (k=i, l=j; k < nval1 && l < nval2; ){
                  ...
                  if (diff < min_gap){
                     ...
                     ctr = it1->mcnt(k/2);
                     if (fl){
                        //cout << "ADD " << icid << " " << it2->ival(l+1)
                        //     << endl;
                        mary->add(icid);
                        mary->add(it2->ival(l+1));
                        mctr.add(0);
                        fl = false;
                     }
                     //cout << "ctr " << ctr << endl;
                     mctr[mctr.size()-1]+=ctr;
                     mcnt+=ctr;
                     ClassInfo::TMPM[ClassInfo::getcls(icid)]+=ctr;
                     k+=2;
                  }
                  if (k >= nval1){
                     k = kpos;
   ...
   if (ljoin){
      ...
      for (i=0; i < NUMCLASS; i++){
         if (ClassInfo::TMPL[i] >= ClassInfo::MINSUP[i]){
            ...
            make_itemset(ljoin, lary, lcnt, ClassInfo::TMPL, lctr);
            break;
         }
      }
   }
   if (ejoin){
      ...
      for (i=0; i < NUMCLASS; i++){
         if (ClassInfo::TMPE[i] >= ClassInfo::MINSUP[i]){
            ...
            make_itemset(ejoin, eary, ecnt, ClassInfo::TMPE, ectr);
            break;
         }
      }
   }
   if (mjoin){
      ...
      for (i=0; i < NUMCLASS; i++){
         if (ClassInfo::TMPM[i] >= ClassInfo::MINSUP[i]){
            ...
            make_itemset(mjoin, mary, mcnt, ClassInfo::TMPM, mctr);
            break;
         }
      }
   }
